...
type SortableItemProps = {
  ...
  onInputChange: (
    position: number,
    field: keyof Omit<MedicationForm, "position">,
    value: string
  ) => void;
};

// SortableItem component for each medication card
const SortableItem: React.FC<SortableItemProps> = ({ form, onRemove, onInputChange }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <Card
        className={`p-4 bg-gray-50 border border-gray-200 hover:border-blue-400 transition-all duration-300 ${isDragging ? 'shadow-lg' : ''
          }`}
      >
