...
import { useUploadFile } from "../../api/medicalRecords/upload-file-records";
...
const DocumentList = () => {
	...
	console.log("listDocument", listDocuments);
	...
	const handleFileUpload = async (position: number, file: File): Promise<void> => {
		if (!file) return;
		setPosition(position);
		uploadFile(file);
	};
	const onFileChange = (position: number, file: File | null | undefined): void => {
		if (file) {
